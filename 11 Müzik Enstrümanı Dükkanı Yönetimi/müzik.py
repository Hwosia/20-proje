...
class stockyonetim:
    def __init__(self, root):
        ...
        ttk.Button(self.stock_sorgula_frame, text="Sipariş Temizle", command=self.siparis_temizle).grid(row=3, column=4, padx=10, pady=10, sticky="we")
        ...
        self.stock_temizle_btn.grid(row=3, column=3, padx=10, pady=10, sticky="we")
                # Sipariş Yönetimi ve Destek Talebi Butonları
        ttk.Button(self.stock_sorgula_frame, text="Sipariş Yönetimi", command=self.siparis_yonetimi_goster).grid(row=3, column=2, padx=10, pady=10, sticky="we")
        ttk.Button(self.stock_sorgula_frame, text="Destek Talebi", command=self.destek_talebi_goster).grid(row=3, column=3, padx=10, pady=10, sticky="we")
    ...
    def siparis_gonder(self, urun_ismi, miktar):
        # Stoktan miktarı düş
        self.stok_miktarini_dus(urun_ismi, miktar)

        # Güncellenmiş stoku göster
        self.stok_guncelle()

        # Sipariş verildiğini bildir
        messagebox.showinfo("Başarılı", "Sipariş verildi.")

    def stok_miktarini_dus(self, urun_ismi, miktar):
        # Stoktan miktarı düşme işlemlerini gerçekleştir
        # Örnek olarak, burada stok veritabanından miktarı düşebilirsiniz
        pass  # Stoktan miktarı düşme işlemlerini buraya ekle

    def stok_guncelle(self):
        # Stok güncelleme işlemlerini gerçekleştir
        # Örneğin, mevcut stok miktarını alabilir ve ekranda gösterebilirsiniz
        # Burada güncel stok miktarını etkileşimli bir şekilde göstermek için 
        # Treeview bileşenine veya bir etikete güncel stok miktarını yerleştirebilirsiniz
        # Örnek olarak, Treeview bileşenini güncelleyebilirsiniz
        self.urun_liste.delete(*self.urun_liste.get_children())  # Önceki verileri temizle
        self.cursor.execute("SELECT * FROM stok")
        urunler = self.cursor.fetchall()
        for urun in urunler:
            self.urun_liste.insert("", "end", values=urun)
    ...
    def destek_talebi_goster(self):
        if not self.kullanici_adi:
            messagebox.showwarning("Uyarı", "Stock sorgulama işlemi için önce giriş yapmalısınız!")
            return
        # Implement support request functionality here
        pass

    def siparis_temizle(self):
        if not self.kullanici_adi:
            messagebox.showwarning("Uyarı", "Stock sorgulama işlemi için önce giriş yapmalısınız!")
            return
        ...
        if not self.urun_liste.selection():
            messagebox.showwarning("Uyarı", "Lütfen önce bir ürün seçiniz.")
            return

        secili_urun = self.urun_liste.item(self.urun_liste.selection()[0])['values'][1]

        # Uyarı mesajı göster
        cevap = messagebox.askquestion("Uyarı", f"{secili_urun} ürününü silmek istiyor musunuz?")

        if cevap == 'yes':  # Eğer kullanıcı evet derse
            # Seçilen ürünü sil
            secili_urun_id = self.urun_liste.item(self.urun_liste.selection()[0])['values'][0]
            self.urun_liste.delete(self.urun_liste.selection()[0])
            messagebox.showinfo("Başarılı", f"{secili_urun} ürünü başarıyla silindi.")

            # Veritabanından da ürünü sil
            self.cursor.execute("DELETE FROM stok WHERE ID=?", (secili_urun_id,))
            self.connection.commit()
        else:
            # Eğer kullanıcı hayır derse
            messagebox.showinfo("Bilgi", "Ürün silinmedi.")
