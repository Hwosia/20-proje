...
class KursUygulamasi(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Egitim Materyali ve Online ders Platformu")
        cursor.execute("CREATE TABLE IF NOT EXISTS materyaller (materyal_id INTEGER PRIMARY KEY, materyal_adi TEXT)")
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        self.check_login_and_create_soru_sorma_tab()  
        self.check_login_and_create_materyaller_tab()
    ...
    def create_materyaller_tab(self):
        self.materyaller_tab = ttk.Frame(self.tabControl)
        self.tabControl.add(self.materyaller_tab, text="Materyaller")

        self.materyaller_tab_label = ttk.Label(self.materyaller_tab, text="Materyaller")
        self.materyaller_tab_label.pack(pady=10)

        # Materyal Yükleme Butonu
        self.materyal_yukle_button = ttk.Button(self.materyaller_tab, text="Materyal Yükle", command=self.materyal_yukle)
        self.materyal_yukle_button.pack(pady=5)

        # Materyale Erişim Butonu
        self.materyal_erisim_button = ttk.Button(self.materyaller_tab, text="Materyale Erişim", command=self.materyal_erisim)
        self.materyal_erisim_button.pack(pady=5)

        # Materyali Silme Butonu
        self.materyal_sil_button = ttk.Button(self.materyaller_tab, text="Materyali Sil", command=self.materyal_sil)
        self.materyal_sil_button.pack(pady=5)

        # Panel için Treeview widget'ı oluştur
        self.panel = ttk.Treeview(self.materyaller_tab, columns=('ID', 'Materyal Adı'))
        self.panel.heading('#0', text='ID')
        self.panel.heading('#1', text='Materyal Adı')
        self.panel.pack(expand=True, fill='both')

    def check_login_and_create_materyaller_tab(self):
        # Giriş yapılmışsa materyaller sekmesini oluştur
        if self.is_user_logged_in():  # Kullanıcı giriş yapmış mı kontrolü
            self.create_materyaller_tab()

    def is_user_logged_in(self):
        # Kullanıcının giriş yapıp yapmadığını kontrol eden bir fonksiyon
        # Burada giriş yapıldı mı kontrolünü yapabilirsiniz
        # Örneğin, kullanıcının bir oturum açıp açmadığını kontrol edebilirsiniz
        # Bu örnekte basit bir kontrol yapısı kullanacağız:
        # Giriş yapıldıysa True, aksi halde False dönecek.

        # Örneğin, e-posta adresi girilmişse giriş yapılmış kabul edelim:
        return bool(self.giris_email_entry.get())

    def materyal_yukle(self):
        # Materyal adını al
        materyal_adi = simpledialog.askstring("Materyal Yükle", "Materyal Adı:")

        # Materyali veritabanına kaydet
        cursor.execute("INSERT INTO materyaller (materyal_adi) VALUES (?)", (materyal_adi,))
        conn.commit()

        messagebox.showinfo("Başarılı", "Materyal başarıyla yüklendi.")

        # Materyalleri güncelle
        self.update_materyal_panel()

    def materyal_erisim(self):
        # Seçili materyalin adını al
        secili_materyal = self.panel.item(self.panel.selection())['values']
        if secili_materyal:
            materyal_adi = secili_materyal[1]
            messagebox.showinfo("Erişim Sağlandı", f"{materyal_adi} materyaline erişim sağlanıyor.")
        else:
            messagebox.showerror("Hata", "Lütfen bir materyal seçin.")

    def materyal_sil(self):
        # Seçili materyalin adını al
        secili_materyal = self.panel.item(self.panel.selection())['values']
        if secili_materyal:
            materyal_id = secili_materyal[0]

            # Materyali veritabanından sil
            cursor.execute("DELETE FROM materyaller WHERE materyal_id = ?", (materyal_id,))
            conn.commit()

            messagebox.showinfo("Başarılı", "Materyal başarıyla silindi.")
    ...
    def update_materyal_panel(self):
        # Materyal panelini güncelleyen bir fonksiyon
        # Veritabanından materyalleri al
        cursor.execute("SELECT * FROM materyaller")
        materyaller = cursor.fetchall()

        # Paneli temizle
        self.panel.delete(*self.panel.get_children())

        # Materyalleri panele ekle
        for materyal in materyaller:
            self.panel.insert('', tk.END, values=materyal)

    def create_soru_sorma_tab(self):
        self.soru_sorma_tab = ttk.Frame(self.tabControl)
        self.tabControl.add(self.soru_sorma_tab, text="Soru Sor")

        self.soru_sorma_button = ttk.Button(self.soru_sorma_tab, text="Soru Sor", command=self.soru_sor)
        self.soru_sorma_button.pack(padx=10, pady=10)

        self.soru_listbox = tk.Listbox(self.soru_sorma_tab, width=70, height=15)
        self.soru_listbox.pack(padx=10, pady=10)

        self.cevap_button = ttk.Button(self.soru_sorma_tab, text="Cevap", command=self.cevap_ver)
        self.cevap_button.pack(padx=10, pady=10)

    def check_login_and_create_soru_sorma_tab(self):
        if self.is_user_logged_in():
            self.create_soru_sorma_tab()

    def soru_sor(self):
        soru = simpledialog.askstring("Soru Sor", "Sorunuzu girin:")
        if soru:
            self.soru_listbox.insert(tk.END, soru)

    def cevap_ver(self):
        selected_item = self.soru_listbox.curselection()
        if selected_item:
            messagebox.showinfo("Cevap Ver", "Soru cevaplanmadı.")
        else:
            messagebox.showinfo("Cevap Ver", "Cevaplanacak bir soru seçilmedi.")
    ...
    def giris(self):
        ...
        if kullanici:
            ...
            self.check_login_and_create_materyaller_tab()
            self.check_login_and_create_soru_sorma_tab() # Materyaller sekmesini oluştur
    ...
    def create_kurs_tab(self):
        ...
        self.cikis_button = ttk.Button(self.kurs_tab, text="Çıkış", command=self.quit)
